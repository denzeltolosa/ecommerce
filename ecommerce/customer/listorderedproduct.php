...
<?php 
	 $query = "SELECT * FROM `tblsummary` s ,`tblcustomer` c 
		...
		if($cur->ORDEREDSTATS=='Pending'){

		
		if ($cur->PAYMENTMETHOD=="Cash on Pick Up") {
...
?>
 	 <h4>Your order has been placed! Please wait for the shop to confirm your order!</h4><br/>
 		<h5>Dear Ma'am/Sir ,</h5>
		<h5>Thank you for ordering! As you have ordered via Cash on Delivery, please have the exact amount of cash on the delivery day.	</h5>
		 <hr/>
...
elseif ($cur ->PAYMENTMETHOD=='GCASH') {
	echo "The payment for the item is complete, please wait for the confirmation of the order";
}

}elseif($cur->ORDEREDSTATS=='Confirmed'){

	 echo "Your order has been confirmed";
}elseif($cur->ORDEREDSTATS=='Prepared'){

	 echo "The shop is preparing your item";
...
}elseif($cur->ORDEREDSTATS=='Delivering'){

	 echo "Your order is on its way";

}elseif($cur->ORDEREDSTATS=='Return'){

	 echo "The item has been return to the owner due to damage item/customer not available";

}
elseif ($cur ->ORDEREDSTATS=='Delivered'){
	echo "<h5>Your order has been delivered.</h5>";
}
?>
				 ...
				 $subtot=0;
				  $query = "SELECT * 
						...
						echo '<tr>';  
				  		...
				  		echo '<td>'. $result->ORDEREDQTY.'</td>';
				  		echo '<td> &#8369 ' .number_format($result->ORDEREDPRICE,2).' </td>'
				  		?>
		...
		?>
 		...
 		<div class="row">
		  ...
		  <div class="row">
		  	  ...
		  	  <p>Sincerely.</p>
		  	  <h4><a href="https://bit.ly/2LPn9Wu">Ramdale</a></h4>
		  </div>
		  <?php }?>
  </div> 
...
function tablePrint(){  
	...
	$(document).ready(function() {
		oTable = jQuery('#appointment').dataTable({
		"bJQueryUI": true,
