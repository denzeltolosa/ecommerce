	...
	function doEdit(){
		...
		if ($_GET['actions']=='confirm') {
				...
				$delivered = Date('Y-m-d');
							...
							# code...
				$status	= 'Delivering';	
				$remarks ='Your order is on its way.';
				$delivered = Date('Y-m-d');
		}elseif ($_GET['actions']=='prepare') {
							# code...
				$status	= 'Prepared';	
				$remarks ='The shop is preparing your item.';
				$delivered = Date('Y-m-d');
		}elseif ($_GET['actions']=='complete') {
							# code...
				$status	= 'Delivered';	
				$remarks ='Your order has been delivered.';
				$delivered = Date('Y-m-d');
		}elseif ($_GET['actions']=='return') {
							# code...
				$status	= 'Return';	
				$remarks ='The item has been returned to the owner due to damaged product/customer not available.'; 
  	...
  	// 		$res = $customer->single_customer($_GET['customerid']); 
	        ...
	        $sql = "INSERT INTO `messageout` (`Id`, `MessageTo`, `MessageFrom`, `MessageText`) VALUES (Null, '".$cur->PHONE."','Ramdale','FROM Sto. Tomas : Your order has been '".$status. "'. The amount is '".$cur->PAYMENT. "')";
	        $mydb->setQuery($sql);
	        $mydb->executeQuery();
			...
			foreach ($cur as $result) {
	  		 ...
	  		 $sql = "INSERT INTO `messageout` (`Id`, `MessageTo`, `MessageFrom`, `MessageText`) VALUES (Null, '".$result->OWNERPHONE."','Ramdale','FROM Sto. Tomas : Your  product has been ordered'. The amount is '".$result->PAYMENT. "')";
	        $mydb->setQuery($sql);
	        $mydb->executeQuery();
