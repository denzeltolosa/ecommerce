 	    ...
 	    <div class="row">
		  <div class="col-md-6">
				<div class="form-group input-group"> 
	                ...
	                <span class="input-group-btn" style="right: 23px; top: 12px;">
	                    <i class="fa fa-calendar" ></i> 
	                </span>
	            </div>
				</div>
...
$query="SELECT *,SUM(ORDEREDQTY) as 'QTY'  FROM `tblproduct` P  ,`tblpromopro` PR ,`tblorder` O, `tblsummary` S ,`tblcustomer` C
WHERE P.`PROID`=PR.`PROID` AND PR.`PROID`=O.`PROID` AND O.`ORDEREDNUM`=S.`ORDEREDNUM` AND S.`CUSTOMERID`=C.`CUSTOMERID` AND ORDEREDSTATS='Delivered' 
AND  DATE(ORDEREDDATE) >= '". date_format(date_create($_POST['date_pickerfrom']),'Y-m-d')."' 
AND DATE(ORDEREDDATE) <= '". date_format(date_create($_POST['date_pickerto']),'Y-m-d')."' GROUP BY `ORDERID` ";
